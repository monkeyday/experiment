...
function Checkbox(p) {
  ...
  r.forEach((v, i) => {
    a.push(
      <label className='checkbox-inline' key={v + b[i].m_code}>
        <input id={v} name='p_code' type='checkbox' value={v} onClick={p.f} />{b[i].p_name}
      </label>
    );
  });
  return <div className='checkbox'>{a}</div>;
}

function Form(p) {
  return (
    <form id='filter' className='form-inline'>
      <Checkbox d={p.d} f={p.f} />
    </form>
  );
}
...
function Point(p) {
  ...
  console.log(d);
  return (
    <circle cx={d.x} cy={d.y} r={d.r} fill={d.f} stroke={d.s} strokeWidth={d.b} />
  );
}
...
class Graph extends React.Component {
  ...
  render() {
    ...
    return (
      <div className="row">
      <Form d={this.state.D} f={this.click} />
      ...
      <g>
        ...
        <Point d={{ x: 150, y: 150, r: 1, f: 'black', s: 'black', b: 1 }} />
        ...
        <Text s={this.state.D} xS={this.xS('p_code')} yS={this.yS('avg_price')} d={{ x: [100, 5] , y: [100, -5], f: 'blue', s: 'white', b: 1, d: '5,4', c: 'Hello', fs: 36, o: 0.5 }} />
        <Path {...path} />
      <g id='x'></g>
      <g id='y'></g>
